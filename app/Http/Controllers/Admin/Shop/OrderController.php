...
class OrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $refund_imgs = explode('|', $order->refund_imgs);

        return view('admin.shop.order.show', compact('order', 'expresses', 'refund_imgs'));
    }
     ...
     * @return array
     */
    public function success_refund(Request $request)
    {
        $order = Order::find($request->id);
        $order->status = 6;
        $order->success_time = date('Y-m-d H:m:s', time());
        $order->save();
     ...
     * @return array
     */
    public function fail_refund(Request $request)
    {
        $order = Order::find($request->id);
        $order->status = 7;
        $order->fail_time = date('Y-m-d H:m:s', time());
        $order->save();
